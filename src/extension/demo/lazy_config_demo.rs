...
use duckfn::{
    DuckLazy, DuckOptionResult, DuckResult, DuckStruct, DuckValueReader, DuckValueType,
...
/// lazy 组的状态：**缓存解析结果**，而不是缓存凭证。
///
/// The lazy group's state: it caches the *parsed value*, never the token.
#[derive(Default, Debug, Clone)]
pub struct LazyConfigAggState {
    config: Option<LazyDemoConfig>,
    rows: i64,
    weighted_sum: f64,
}
...
impl duckfn::DuckAggregateState for LazyConfigAggState {
    ...
    fn simple_combine(&mut self, other: &Self) {
        // 配置项不必重新解析：两边解析出来的内容一样，直接复用另一边的结果。
        //
        // No re-parse is needed: both sides parsed the same configuration, so reuse it.
    ...
    fn simple_result(&self) -> Self::Output {
        LazyConfigAggOutput {
            ...
            scale: self.config.as_ref().map_or(0.0, |c| c.0.scale),
        }
    }
}
...
fn dfn_agg_lazy_config_demo(
    ...
    let config = state.config.as_ref().expect("config was just parsed");

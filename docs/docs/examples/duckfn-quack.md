...
sidebar_position: 1
description: duckfn_quack, the example extension shipped with duckfn, with runnable SQL for every feature duckfn supports.
---

# The example extension

`duckfn-quack/` holds a complete DuckDB extension called `duckfn_quack`. It is not published on its
own — it exists to exercise every feature, and its sqllogictest suite under `duckfn-quack/test/sql/`
is the reference for what each function returns. It also ships inside the published `duckfn`
package, so its sources and tests are readable without cloning the repository.

## Build and load

```bash
make configure   # once: prepares the Python venv used by the test runner
make debug       # -> build/debug/extension/duckfn_quack/duckfn_quack.duckdb_extension
```

```bash
duckdb -unsigned -c "
LOAD './build/debug/extension/duckfn_quack/duckfn_quack.duckdb_extension';
...
| Path | Contents |
| --- | --- |
| `duckfn-quack/src/extension/mod.rs` | `duckfn_entrypoint!("duckfn_quack")` and the module tree. |
| `duckfn-quack/src/extension/demo/` | One file per feature area, including several hand-written FFI variants kept for comparison. |
| `duckfn-quack/src/extension/functions/` | One file per registration kind: scalar, aggregate, table, cast, replacement scan, SQL macro. |
| `duckfn-quack/src/extension/functions/sql/` | `.sql` files registered through `include_str!` and `duck_sql_macro_files!`. |
| `duckfn-quack/src/extension/types/` | Echo functions for every supported type, in scalar and table form. |
| `duckfn-quack/test/sql/` | 41 sqllogictest files mirroring the source layout. |

`duckfn-quack/src/extension/demo/rewrite_official_template_demo.rs` is the smallest useful starting point — it is
...
`duckfn-quack/test/sql/` mirrors the source layout and is the most precise description of behaviour
available:

```
duckfn-quack/test/sql/

...
## `rusty_echo(varchar) -> varchar`

<CodeCompare
  leftTitle="duckdb crate — VScalar"
  rightTitle="duckfn"
  left={`
    struct EchoScalar;
...
## `rusty_quack(varchar) -> table(column0 varchar)`

<CodeCompare
  leftTitle="duckdb crate — VTab"
  rightTitle="duckfn"
  left={`
    #[repr(C)]
...
## `word_count(varchar) -> bigint` (aggregate)

<CodeCompare
  leftTitle="quack-rs — AggregateFunctionBuilder"
  rightTitle="duckfn"
  left={`
...
## `first_word(varchar) -> varchar`

<CodeCompare
  leftTitle="quack-rs — ScalarFunctionBuilder"
  rightTitle="duckfn"
  left={`
...
- [The example extension](./rusty-quack.md) — the same ideas covering every feature duckfn has.

...
下的文档正文及其简体中文译文，所以用户指南与那页可以照着跑的示例页都能直接从解包后的 crate 里读 ——
见[构建与发布](../build-and-release.md)。

包带不过去的是它们周围的那一整套仓库：示例扩展的源码、issue 历史、CI 工作流、文档站自己的工具链，
以及 `AGENTS.md` 这个文件本身。所以 `AGENTS.md` 把知识源指向本仓库的本地 clone，并明确要求助手查不到时
就停下来问，而不是凭印象编一个属性出来 —— 两个占位符里有一个就是它的路径。

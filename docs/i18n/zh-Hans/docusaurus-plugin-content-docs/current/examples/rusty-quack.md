...
title: 示例扩展
sidebar_position: 1
description: rusty_quack —— 本仓库的示例扩展，用可运行的 SQL 覆盖 duckfn 的每一项功能。
---

# 示例扩展

仓库根目录就是一个完整的 DuckDB 扩展 `rusty_quack`。它不发布，存在的意义是把每一项功能都跑一遍；
`test/sql/` 下的 sqllogictest 用例则是各函数行为的权威依据。

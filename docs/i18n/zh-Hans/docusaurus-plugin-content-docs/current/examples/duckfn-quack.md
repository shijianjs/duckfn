...
title: 示例扩展
sidebar_position: 1
description: duckfn_quack —— 随 duckfn 一起发布的示例扩展，用可运行的 SQL 覆盖 duckfn 的每一项功能。
---

# 示例扩展

`duckfn-quack/` 目录是一个完整的 DuckDB 扩展 `duckfn_quack`。它不单独发布，存在的意义是把每一项功能
都跑一遍；`duckfn-quack/test/sql/` 下的 sqllogictest 用例则是各函数行为的权威依据。它会随已发布的
`duckfn` 包一起分发，因此不用克隆仓库就能读到它的源码与测试。
...
| 路径 | 内容 |
| --- | --- |
| `duckfn-quack/src/extension/mod.rs` | `duckfn_entrypoint!("duckfn_quack")` 与模块树。 |
| `duckfn-quack/src/extension/demo/` | 每个功能域一个文件，另有若干手写 FFI 版本用于对照。 |
| `duckfn-quack/src/extension/functions/` | 每种注册方式一个文件：标量、聚合、表函数、类型转换、替换扫描、SQL 宏。 |
| `duckfn-quack/src/extension/functions/sql/` | 通过 `include_str!` 与 `duck_sql_macro_files!` 注册的 `.sql` 文件。 |
| `duckfn-quack/src/extension/types/` | 每种受支持类型的 echo 函数，标量与表函数两种形式。 |
| `duckfn-quack/test/sql/` | 41 个 sqllogictest 文件，与源码目录一一对应。 |

`duckfn-quack/src/extension/demo/rewrite_official_template_demo.rs` 是最小的可用起点 —— 它是把 DuckDB 官方的模板示例
用 duckfn 重写了一遍：
...
`duckfn-quack/test/sql/` 与源码目录一一对应，是关于行为最精确的描述：

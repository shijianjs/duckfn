...
| `#[derive(DuckStruct)]` | Map a struct to a DuckDB `STRUCT` (optionally creating the type at load time with `create_type = true`, or only printing the DDL with `create_type = "print"`). |
| `#[derive(DuckEnum)]` | Map a unit-variant enum to a DuckDB `ENUM` (optionally creating the type at load time with `create_type = true`, or only printing the DDL with `create_type = "print"`). |

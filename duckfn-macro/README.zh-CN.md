...
| `#[derive(DuckStruct)]` | 把结构体映射为 DuckDB `STRUCT`（可选用 `create_type = true` 在加载期建类型，或用 `create_type = "print"` 只打印 DDL）。 |
| `#[derive(DuckEnum)]` | 把只有单元变体的枚举映射为 DuckDB `ENUM`（可选用 `create_type = true` 在加载期建类型，或用 `create_type = "print"` 只打印 DDL）。 |
